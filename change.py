    ...
    for data in datas:
        ...
        if month == 12:
            ...
            if (data.get('sex') == '男' and year-year2 == 55) or (data.get('sex') == '女' and year-year2 == 45):
                ...
                if int(mon) + rmb < 10:
                    data["end_month"] = '{}0{}'.format(year, int(mon) + rmb)
                else:
                    data["end_month"] = '{}{}'.format(year, int(mon) + rmb)
            elif (data.get('sex') == '男' and year-year2 == 60) or (data.get('sex') == '女' and year-year2 == 55):
                data["start_month"] = f'{year}01'
                if rmb < 10:
                    data["end_month"] = '{}0{}'.format(year, rmb)
                else:
                    data["end_month"] = '{}{}'.format(year, rmb)
            else:
                data["start_month"] = f'{year}01'
                if rmb < 10:
                    data["end_month"] = '{}0{}'.format(year, rmb)
